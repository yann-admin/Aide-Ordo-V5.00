...
import { flipFlopType } from "./module/flipFlopType.js";
/* ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑ Import  ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑ */
...
// We select the element with the id "div-msg" outside the modal, which will be used to display messages (e.g., success or error messages) to the user after form submission. This allows us to show messages in a more prominent location on the page, rather than within the modal itself.
const divMsg = document.querySelector('#div-msg');

// We add a click event listener to the eye icon, which triggers the flipFlopType function when clicked. This allows users to toggle the visibility of the password input field between text and password types.
const pictoEye = document.querySelector('.picto-eye');
...
window.addEventListener("load", () => {
    ...
    myModalPrimarySubmitBtn.addEventListener('click', function (event) {
         ...
         *  @param {HTMLElement} divMsg - The element where the response message will be displayed
         * @param {number|null} tOnSuccess - Optional timeout in milliseconds before redirecting on success (default: null, which means immediate redirect)
         */
        handleFormSubmit(myModalPrimaryForm, myModalPrimarySubmitBtn.dataset.url, divMsg, 500); 
    });
    ...
    // We add a click event listener to the eye icon, which triggers the flipFlopType function when clicked. This allows users to toggle the visibility of the password input field between text and password types.
    pictoEye.addEventListener('click', function () {
        flipFlopType(passwordInput, pictoEye);
    });
